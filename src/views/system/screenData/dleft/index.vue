  ...
  <div class="left-content">
    <div class="left-box">
      <databox :title="''" :dheight="720">
        ...
        <databox
          :title="$t('data.dleft.device_info')"
          :dheight="280"
          :icon="'account'"
          :boxb="false"
        >
        ...
        <databox
          :title="$t('data.dleft.device_list')"
          :dheight="280"
          :icon="'account'"
          :boxb="false"
        >
...
export default {
  ...
  methods: {
    getData (username) {
      this.$axios
        .get(`/api/screenDevice?token=${cookieGet("token")}`)
        .then(response => {
          ...
          if (data.length < 1) {
            ...
            //重新组装语言封成对象
            let camera = JSON.parse(JSON.stringify(response.camera))
            let dataL = []
            for (var j = 0; j < camera.length; j++) {
              let lang = camera[j].name
              let number = camera[j].count
              let obj = {
                lang: lang,
                number: number
              }
